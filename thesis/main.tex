...
\kapitola{Discussion}
Work on this thesis has shown numerous opportunities for further development. This could be
attributed to the broad scope of the thesis topic. I this section achieved results will be summarized and directions for further implementation proposed.

\sekce{Improved peripheral support}
During implementation of the thesis \textit{PWM} support was added to the MicroPython's Zephyr 
port with community developed port extension and hardware configuration via 
\textit{Devicetree}. Tho the \textit{PWM} support was added some other means hardware options 
are still not properly supported or the supported parts do not use Zephyrs API for speed and
reliability.

A potential extension would be to implement \textit{I2C, I3C} support on MicroPython's Zephyr 
port.

\sekce{Improved modules compatibility}
Although during the implementation phase of this thesis more modules were added to the 
MicroPython's Zephyr port those lacked use of efficient Zephyr APIs.

Rewrites of this modules for higher speed and better hardware integration could be a potential
improvement. 

\sekce{Integration with ports configuration}
During implementation of this thesis significant reconfiguration was introduced but this 
changes were not visible during the port use and knowledge about the final build and 
configuration is available only to the developer who made the configuration changes.

A module highlighting the current configuration and available modules could be implemented, 
which would potentially ease of further development saving time on following big an numerous
configuration files.
...
sample program and observing its correct work.
...
tested by loading to the board and observing the behavior of a sample program.
